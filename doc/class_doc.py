...
class ClassDoc:
    ...
    docstring_re = re.compile(r"(\"\"\"(?P<content>.*?)\"\"\")", flags=re.DOTALL)
    method_re = re.compile(
        r"^ +def (?P<method>(__|[a-z])[A-Za-z_0-9]*)" "\((?P<args>.*?)\):\n",
    ...
    def source_url(self):
        return "{}/{}#L{}".format(
            doc_config.SOURCE_ROOT, self.parent.path, self.line_num
        )
    ...
    def normal_methods(self):
        return [
            ...
            if method.method_type == MethodType.normal and method.name != "__init__"
        ]
    ...
    def inherited_init_method(self):
        return next(
            (method for method in self.inherited_methods if method.name == "__init__"),
            None,
        )
    ...
    def parse_class(self):
        ...
        for docstring in docstrings:
            ...
            method_match = first_or_none(
                m for m in methods if m.end(0) - 1 == last_line_end_i
            )
            attribute_match = first_or_none(
                a for a in attributes if a.end(0) - 1 == last_line_end_i
            )
            if method_match:
                ...
                if ClassDoc.property_re.search(line_before_method):
                    self.properties.append(
                        AttributeDoc(
                            ...
                            None,
                            self.global_index,
                            self.body_start_line_num
                            + line_num_at(method_match.start(0), self.body)
                            - 1,
                        )
                    )
                else:
                ...
                self.class_attributes.append(
                    AttributeDoc(
                        ...
                        self.global_index,
                        self.body_start_line_num
                        + line_num_at(attribute_match.start(0), self.body)
                        - 1,
                    )
                )
            else:
    ...
    def find_inherited_names(self):
        ...
        for superclass in self.superclasses:
            ...
            ) = superclass.find_inherited_names()
            inherited_methods = {
        ...
        return (inherited_methods, inherited_properties, inherited_class_attributes)
    ...
    def discover_inheritence(self):
        (
            ...
            inherited_properties,
            inherited_class_attributes,
        ) = self.find_inherited_names()
        all_inherited = (
            list(inherited_methods.values())
        ...
        for inherited in all_inherited:
            if type(inherited).__name__ == "MethodDoc":
                ...
                type(inherited).__name__ == "AttributeDoc" and not inherited.is_property
            ):
    ...
    def resolve_names_and_parse_html(self):
        ...
        if self.superclass_string:
            ...
            for superclass in self.superclass_string.split(", "):
                superclasses.append(parse_type_and_add_code_tag(superclass, self))
