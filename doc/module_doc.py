...
class ModuleDoc:
    ...
    class_re = re.compile(
        r"^class " + class_name_re_capture + r"(\((?P<superclasses>.*?)\))?" + r":\n",
    ...
    def source_url(self):
        ...
        return "{}/{}".format(doc_config.SOURCE_ROOT, path)
    ...
    def parse_module(self):
        ...
        contents = "".join(lines)
        class_matches = list(re.finditer(ModuleDoc.class_re, contents))
        method_matches = list(re.finditer(ModuleDoc.module_level_method_re, contents))
        attribute_matches = list(
            re.finditer(ModuleDoc.module_level_attribute_re, contents)
        )
        ...
        for block_index, block in enumerate(docstring_blocks):
            last_line_end_i = previous_line_ending_index_from(block.start(0), contents)
            docstring = block.group("content")
            class_match = first_or_none(
                c
                for c in class_matches
                # Allow a blank line before class docstring
                if c.end(0) - 1 == last_line_end_i or c.end(0) == last_line_end_i
            )
            method_match = first_or_none(
                m for m in method_matches if m.end(0) - 1 == last_line_end_i
            )
            attribute_match = first_or_none(
                a for a in attribute_matches if a.end(0) - 1 == last_line_end_i
            )
            if class_match:
                class_body = everything_in_indentation_block(block.end(0), contents)
                self.classes.append(
                    ClassDoc(
