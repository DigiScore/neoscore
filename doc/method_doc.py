...
class MethodDoc:
    ...
    def __init__(
        self, name, parent, args_string, docstring, method_type, global_index, line_num
    ):
    ...
    def source_url(self):
        ...
        return "{}/{}#L{}".format(doc_config.SOURCE_ROOT, path, self.line_num)
    ...
    def parse_repeating_type_block(self, block_match, arg_mode=False):
        ...
        def parse_type_and_explanation(string):
            ...
            if ":" in string:
                if arg_mode:
                    ...
                    type_parsed_string = re.sub(
                        r"(?P<content>.*?)\:", re_type_sub_add_colon, remainder, 1
                    )
                    recombined = arg_name + " " + type_parsed_string
                    return parse_general_text(recombined, self.parent)
                else:
                    type_parsed_string = re.sub(
                        r"(?P<content>.*?)\:", re_type_sub_add_colon, string, 1
                    )
        ...
        current_item = ""
        for line in lines:
            if indentation_level_at(0, line) == main_indentation_level and current_item:
    ...
    def resolve_names_and_parse_html(self):
        current_i = 0
        while current_i < len(self.docstring) and self.docstring[current_i] != "\n":
            current_i += 1
        ...
        details_end = min(
            (
                match.start(0)
                for match in [args_block, returns_block, warning_block, raises_block]
                if match
            ),
        ...
        if args_block:
            self.args_details = self.parse_repeating_type_block(
                args_block, arg_mode=True
            )
        ...
        self.args_details = [
            parse_general_text(arg, self.parent) for arg in self.args_details
        ]
        self.returns = [parse_general_text(ret, self.parent) for ret in self.returns]
        self.exceptions = [parse_general_text(e, self.parent) for e in self.exceptions]
