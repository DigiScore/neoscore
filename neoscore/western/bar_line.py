...
class BarLine(Path, MultiStaffObject):
    ...
    def __init__(self, pos_x: Unit,
                 staves: Staff | Iterable[Staff],
                 barline_style: BarlineStyle,
                 connected: bool):
        """
        Args:
            pos_x: The barline position relative to the topmost staff.
            staves: A staff or collection of them to draw the line across.
            barline_style: The style of a barline e.g. double, dashed (see bar_line_style.py)
            connected: Whether the barline connects across staves.
        """
    ...
    @property
    def barline_style(self) -> BarLineStyle:
        """The style of barline (see bar_line_style.py)"""
        if
