...
class Slur(Path, StaffObject, Spanner):
    ...
    def _draw_path(self):
        ...
        control_1 = ParentPoint(self.staff.unit(1), mid_upper_height, parent=self)
        control_2 = ParentPoint(
            self.end_pos.x - self.staff.unit(1),
            self.end_pos.y + mid_upper_height,
            parent=self.end_parent,
        )
        end = ParentPoint(
            self.end_pos.x, self.end_pos.y + end_height, parent=self.end_parent
        )
        self.cubic_to(
            control_1.x,
            control_1.y,
        ...
        control_1 = ParentPoint(
            self.end_pos.x - self.staff.unit(1),
            self.end_pos.y + mid_height,
            parent=self.end_parent,
        )
        control_2 = ParentPoint(
            self.x + self.staff.unit(1), self.y + mid_height, parent=self
        )
        end = ParentPoint(self.x, self.y, parent=self)
        self.cubic_to(
            control_1.x,
            control_1.y,
            control_2.x,
