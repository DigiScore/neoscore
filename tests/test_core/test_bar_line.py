...
class TestBarLine(unittest.TestCase):
    ...
    def test_path_shape_with_same_staff_x_coords(self):
        ...
        assert bar_line.elements[0].pos == Point(Mm(15), self.staff_2.height)
        assert bar_line.elements[0].parent == self.staff_2

    def test_path_shape_with_different_staff_x_coords(self):
        bar_line = BarLine(Mm(15), [self.staff_1, self.staff_2, self.staff_3])
        assert bar_line.elements[0].pos == Point(Mm(5), self.staff_3.height)
        assert bar_line.elements[0].parent == self.staff_3
