...
image_path = pathlib.Path("/some/file/path.png")
...
class TestImage(unittest.TestCase):
    ...
    def test_init(self):
        ...
        image = Image((Unit(5), Unit(6)), parent, image_path, 2)
    ...
    def test_init_with_str_file_path(self):
        image = Image((Unit(5), Unit(6)), None, str(image_path), 2)
        assert image.file_path == image_path

    def test_scale_setter(self):
        image = Image(ORIGIN, None, image_path)
        image.scale = 3
