...
class TestFlowable(unittest.TestCase):
    ...
    def test_generate_layout_controllers_with_many_lines(self):
        ...
        assert(all(c.flowable == test_flowable
                   for c in test_flowable.layout_controllers))
        assert(test_flowable.layout_controllers[1].local_x == live_width)
    ...
    def testlast_break_at(self):
        test_flowable = Flowable((Mm(10), Mm(0)), Mm(500), Mm(90), Mm(5))
        assert(test_flowable.last_break_at(Mm(140)) ==
               test_flowable.layout_controllers[0])
        assert(test_flowable.last_break_at(Mm(180)) ==
               test_flowable.layout_controllers[1])
