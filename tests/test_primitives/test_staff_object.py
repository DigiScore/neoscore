...
class TestStaffObject(unittest.TestCase):
    ...
    def test_find_root_staff_with_direct_parent(self):
        class StaffObjectClass(StaffObject):
            pass
        staff = Staff((0, 0), 100, self.frame)
        child_object = StaffObjectClass((Mm(0), Mm(0)), Mm(5), staff)
        assert(child_object.root_staff == staff)

    def test_find_root_staff_with_ancestor(self):
    ...
    @unittest.skip
    def test_find_root_staff_with_no_staff_raises_error(self):
