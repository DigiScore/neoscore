...
class TestDocument(unittest.TestCase):
    ...
    def test_live_page_origin_in_doc_space_at_first_page(self):
        left_margin = 13
        top_margin = 21
        test_paper = Paper(200, 250, top_margin, 10, 20, left_margin, 0)
        test_doc = Document(test_paper)
        found = test_doc._live_page_origin_in_doc_space(1)
    ...
    def test_live_page_origin_in_doc_space_with_invalid_page_numbers(self):
        test_paper = Paper(200, 250, 20, 10, 20, 10, 0)
        test_doc = Document(test_paper)
        with pytest.raises(ValueError):
            test_doc._live_page_origin_in_doc_space(0)
        with pytest.raises(ValueError):
            test_doc._live_page_origin_in_doc_space(-1)
