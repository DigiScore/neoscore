...
class TestBrushInterface(unittest.TestCase):
    def test_color_passed_to_qt(self):
        brush = BrushInterface(None, Color(0, 100, 200, 250), BrushPattern.SOLID)
    ...
    def test_brush_pattern_passed_to_qt(self):
        brush = BrushInterface(None, Color(0, 0, 0), BrushPattern.SOLID)
        assert brush.qt_object.style() == BrushPattern.SOLID.value

        brush = BrushInterface(None, Color(0, 0, 0), BrushPattern.DENSE_1)
        assert brush.qt_object.style() == BrushPattern.DENSE_1.value
