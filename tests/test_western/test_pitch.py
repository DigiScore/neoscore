...
class TestPitch(unittest.TestCase):
    ...
    def test_no_octave_marks(self):
        assert Pitch.from_str("af") == Pitch("a", AccidentalType.FLAT, 3)

    def test_comma_octave_marks(self):
        assert Pitch.from_str("af,,,") == Pitch("a", AccidentalType.FLAT, 0)

    def test_apostrophe_octave_marks(self):
        assert Pitch.from_str("af''") == Pitch("a", AccidentalType.FLAT, 5)

    def test_accidental_variants(self):
        assert Pitch.from_str("gs") == Pitch("g", AccidentalType.SHARP, 3)
        assert Pitch.from_str("g#") == Pitch("g", AccidentalType.SHARP, 3)
        assert Pitch.from_str("gn") == Pitch("g", AccidentalType.NATURAL, 3)
        assert Pitch.from_str("gf") == Pitch("g", AccidentalType.FLAT, 3)
        assert Pitch.from_str("gb") == Pitch("g", AccidentalType.FLAT, 3)
        assert Pitch.from_str("gx") == Pitch("g", AccidentalType.DOUBLE_SHARP, 3)
        assert Pitch.from_str("gss") == Pitch("g", AccidentalType.DOUBLE_SHARP, 3)
        assert Pitch.from_str("gff") == Pitch("g", AccidentalType.DOUBLE_FLAT, 3)
        assert Pitch.from_str("gbb") == Pitch("g", AccidentalType.DOUBLE_FLAT, 3)
